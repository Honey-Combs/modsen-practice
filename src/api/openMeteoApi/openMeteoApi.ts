...
export interface IOpenMeteoApi {
...
async function getDailyForecast<IOpenMeteoDailyForecast>({ latitude, longitude }: GPSCoordinates) {
  const { data } = await axios.get<IOpenMeteoDailyForecast>(
    `${BASE_URL}/forecast?latitude=${latitude}&longitude=${longitude}&forecast_days=16&timezone=auto&daily=temperature_2m_max,temperature_2m_min,weathercode,winddirection_10m_dominant,windspeed_10m_max`
  );
...
async function getHourlyForecast({ latitude, longitude }: GPSCoordinates) {
  const response = await fetch(
    `${BASE_URL}/forecast?latitude=${latitude}
  &longitude=${longitude}
...
export const OpenMeteoApi: IOpenMeteoApi = {
  getDailyForecast,
  getHourlyForecast,
};
